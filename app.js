...
async function init(){
  const data=await fetch("nfs-compact.json?v=20260804").then(r=>r.json());
  ...
  let history=[];try{history=await fetch("historico-nfs.json?v=20260804").then(r=>r.ok?r.json():[])}catch{}
  const generalRows=[
